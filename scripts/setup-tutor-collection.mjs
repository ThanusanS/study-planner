/**
 * Appwrite Collection Setup Script — tutorHistory
 *
 * Usage:
 *   node scripts/setup-tutor-collection.mjs
 */
...
const COLLECTION_ID = "tutorHistory";

async function main() {
  console.log("Setting up tutorHistory collection...\n");
  ...
  try {
    ...
    await databases.createEnumAttribute(DATABASE_ID, COLLECTION_ID, "explainType", ["simple", "deep", "doubt"], true);
    console.log("  ✅ explainType (enum: simple/deep/doubt, required)");

    await databases.createStringAttribute(DATABASE_ID, COLLECTION_ID, "tutorContent", 100000, true);
    console.log("  ✅ tutorContent (string, required, max 100k chars)");
    ...
    console.log("\n🎉 tutorHistory collection setup complete!");
